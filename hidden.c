 ...
 */
  ...
  *   the Kernel schedules hidden to hidden them. (the only way around this is
...
\n\
./hidden [-fE] [-n pid] [-a name] command ...\n\
  -a <name>  Rename the process to 'name'. (Use -a- for empty string).\n\
  -f         Hide all child processes as well (follow).\n\
  -E         Do not hide the environment variables.\n\
  -n <pid>   Fast forward to this pid (-n 300 is often the smallest possible)\n\
\n\
Example - Start ssh but hide all options (only 'ssh' shows)\n\
    "CDR"$ "CC"./hidden "CM"ssh"CDM" root@myserver.com"CN"\n\
Example - Start 'nmap', hide all options & make nmap appear as 'harmless':\n\
    "CDR"$ "CC"./hidden "CDC"-a harmless "CM"nmap"CDM" -sCV -F -Pn scanme.nmap.org"CN"\n\
Example - Same but also with the lowest possibl process id:\n\
    "CDR"$ "CC"./hidden "CDC"-a harmless -n0 "CM"nmap"CDM" -sCV -F -Pn scanme.nmap.org"CN"\n\
Example - Start a PHP script as a background daemon. Hidden as 'apache2 -k...'\n\
    "CDR"$ "CDC"("CC"./hidden "CDC"-f -a '/usr/sbin/apache2 -k start' "CM"php"CDM" tool.php "CDC"&>/dev/null &)"CN"\n\
Example - Hide tmux and all child processes as some kernel process:\n\
    "CDR"$ "CC"./hidden"CDC" -f -a '[kworker/1:0-rcu_gp]' "CM"tmux"CN"\n\
Example - Use 'exec' to replace the parent shell as well:\n\
    "CDR"$ "CC"exec ./hidden"CDC" -f -a '[kworker/1:0-rcu_gp]' "CM"tmux"CN"\n\
Example - Hide bash and all child processes (as empty string):\n\
    "CDR"$ "CC"./hidden"CDC" -f -a- "CM"bash"CDM" -il"CN"\n\
Example - Use 'exec' to replace the parent shell as well:\n\
    "CDR"$ "CC"exec ./hidden"CDC" -f -a- "CM"bash"CDM" -il"CN"\n\
\n\
...
do_getopts(int argc, char *argv[])
{
    ...
    // possible: hidden can only claim the new PID by calling fork()
    // and allowing the parent-hidden to exit. This would detach the
    // process from the shell's process group (because parent pid would
    // become 1) and put hidden and/or the target process into the background
...
or start the process in the background:\n\
    ...
    if ((g_cur_prg_name) && (strcmp(argv[0], g_cur_prg_name) != 0)) {
        // argv[0] is still 'hidden'. Execute ourself to fake our own argv[0]
    ...
    while(1) {
        ...
        *pidp = pid;
        ...
        if (WIFSIGNALED(*status)) {
            ...
            if (pid == g_pid_master) {
                if (signum == SIGSEGV)
                    exit(128 + signum); // Do not generate core dump of hidden.
        ...
        if (! (signum & 0x80)) {
            ...
            if (signum == SIGTRAP) {
                ...
                switch ((*status >> 16) & 0xffff) {
                    ...
                    case PTRACE_EVENT_VFORK: ; // 2
                        ...
                        // use './hidden -n 500' within a HIDDEN tmux.
                        ...
                        if (ptrace(PTRACE_CONT, cpid, NULL, NULL) == -1) {
                            DEBUGF("ERROR hidden: ptrace(%ld): %s\n", cpid, strerror(errno));
                            // Child may have exited already
                            // if (errno != EIO)
                                // fprintf(stderr, "ERROR hidden: ptrace(%ld): %s\n", cpid, strerror(errno));
                        }
                        break;
            ...
            // [DEBUG hidden.c:455] Stray SIGSTOP for (untracked?) pid=42056 (status=4991)
            // [DEBUG hidden.c:426] Event for 42054 (event=1)
            // [DEBUG hidden.c:436] FORK 42054 to cpid=42056
            if (signum == SIGSTOP) {
...
start_trace_parent(const char *orig_prog, char *new_argv[]) {
    ...
    // Try hidden to be the CHILD (tracer) and trace the PARENT (tracee)
    // See ptrace_scope
...
fix_stack(pid_t pid)
{
    ...
    if (idx <= 1) {
        ...
        DEBUGF("ERROR hidden: ptrace(%d): %s\n", pid, strerror(errno));
        // fprintf(stderr, "ERROR hidden: ptrace(%d): %s\n", pid, strerror(errno));
        return;
    }
...
start_trace(char *orig_prog, char *new_argv[]) {
    ...
    // $ ./hidden  -a 12345678990abcdef cat
    ...
    if (!(g_flags & FL_FOLLOW)) {
        // Detach after hiding if we are a background process.
